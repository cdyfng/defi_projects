...
const calculateCoins = async (lpAddresses, myAddress, pid) => {
  ...
  const r = {
    token0:
      ((parseInt(reserves._reserve0) / 1e18) * parseInt(lpAmount.amount)) /
      totalSupply,
    token1:
      ((parseInt(reserves._reserve1) / 1e18) * parseInt(lpAmount.amount)) /
      totalSupply,
  };
  //console.log("r:", r);
  return r;
...
async function lhb_routine() {
  try {
    ...
    return { HBTC: s1, MDX: s2, USDT: s3, HT: s4, HUSD: s5, FILE: s6 };
  } catch (e) {
    // statements to handle any exceptions
    console.log(e); // pass exception object to error handler
    return 0;
